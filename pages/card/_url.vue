...
<template>
    <div>
        ...
        <a-scene>
                ...
                <a-sky hide-in-ar-mode id="image-360" rotation="0 -112.49 0" radius="10" src="#city"> </a-sky>
                <a-image
                    look-at="[camera]"
                    src="#my-image"
...
export default {
  layout: "empty",
   async beforeMount(){
    let host = 'https://68d3-178-46-180-106.ngrok.io/api'
      console.log(this.$route.params.url);
      
        await fetch(host + '/' + this.$route.params.url).then(res => res.json()).then(json => {
           this.title = json.title
           this.content = json.content
           this.scene.bg = json.scene.image_url
           this.scene.img = json.scene.position
         })
